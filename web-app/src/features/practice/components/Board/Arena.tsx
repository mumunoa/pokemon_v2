    ...
    const renderPlayerField = (playerId: PlayerId, isOpponent: boolean) => {
        ...
        return (
            <div className={`field ${playerId}-field relative flex flex-col px-2 py-1 overflow-visible ${isOpponent ? 'rotate-180' : ''}`} style={{ flex: isOpponent ? undefined : 'var(--field-f-self)', minHeight: 0 }}>
                        ...
                        <div className="flex flex-col items-center relative">
                            ...
                            <Zone id={`${playerId}-deck` as ZoneType} className="deck-zone horizontal-card w-[var(--card-w)] h-[var(--card-h)] relative border border-solid border-slate-600 flex items-center justify-center hover:border-slate-400 cursor-pointer" onClick={() => setPopupState({ zone: `${playerId}-deck` as ZoneType, viewMode: 'all' })}>
                                {zones[`${playerId}-deck` as ZoneType].length > 0 && (
                                    <>
                                        ...
                                        <div className={`deck-stack player1-card-back w-[90%] h-[92%] absolute bg-slate-700 rounded-md shadow-md ${(isShuffling === playerId || isShuffling === 'both') ? 'animate-shuffle' : ''}`} style={{ backgroundImage: "url('/assets/images/card_back_v4.png')", backgroundSize: "100% 100%", backgroundRepeat: "no-repeat" }} />
                                    </>
                                )}
                            ...
                            {(isDrawing === playerId || isDrawing === 'both') && (
                                <div className="absolute z-[9999] pointer-events-none" style={{
                                    ...
                                    <div className="w-full h-full rounded-md shadow-xl" style={{ backgroundImage: "url('/assets/images/card_back_v4.png')", backgroundSize: "100% 100%", backgroundRepeat: "no-repeat" }} />
                                </div>
                            )}
                ...
                <div className={`bench-row flex justify-center w-full max-w-full mx-auto flex-shrink-0 overflow-x-auto overflow-y-visible scrollbar-hide relative z-[20] ${isOpponent ? 'scale-[var(--row-s-opp-bench)] origin-center' : 'scale-[var(--row-s-self-bench)] origin-center'}`} style={{ minHeight: `calc(var(--card-h) * ${isOpponent ? 'var(--row-s-opp-bench)' : 'var(--row-s-self-bench)'})` }}>
                    <div className="bench-zone flex items-center justify-center gap-[calc(var(--card-gap)*0.5)] p-1 bg-slate-800/40 rounded-lg min-w-max">
                        {benchSlots.map(num => {
                ...
                {isOpponent ? (
                    <Zone id={`${playerId}-hand` as ZoneType} className={`${playerId}-hand-opponent relative w-full flex justify-center space-x-[calc(var(--card-w)*-0.6)] overflow-visible z-[10] opacity-90 flex-shrink-0`} style={{ height: 'calc(var(--card-h) * 0.5)' }}>
                        {zones[`${playerId}-hand` as ZoneType].map((cardId) => (
                            <div key={cardId} className="w-[var(--card-w)] h-[calc(var(--card-h)*1.4)] bg-slate-700 rounded shadow-md border border-slate-600 scale-[0.6] origin-top" style={{ backgroundImage: "url('/assets/images/card_back_v4.png')", backgroundSize: "100% 100%", backgroundRepeat: "no-repeat" }} />
                        ))}

                        {/* Opponent Hand Display - Mirroring player's display */}
                        <div className="absolute bottom-4 left-12 text-slate-400 text-[var(--btn-font-size)] scale-[0.6] origin-left font-bold font-mono bg-slate-900/80 px-3 py-1.5 rounded-full border border-slate-700 shadow-xl z-[100] backdrop-blur-sm rotate-180">
                    ...
                    <Zone id={`${playerId}-hand` as ZoneType} className={`${playerId}-hand relative w-full bg-slate-900/90 border-t border-slate-700 shadow-[0_-5px_15px_rgba(0,0,0,0.5)] flex items-start justify-center space-x-[calc(var(--card-w)*-0.4)] overflow-visible z-50 pb-1 flex-shrink-0`} style={{ height: 'calc(var(--card-h) * var(--hand-h-mult))' }}>
                        ...
                        <div className="absolute bottom-4 right-4 text-slate-400 text-[var(--btn-font-size)] scale-[0.6] origin-right font-bold font-mono bg-slate-900/80 px-3 py-1.5 rounded-full border border-slate-700 shadow-xl z-[70] backdrop-blur-sm">
                            手札 : {zones[`${playerId}-hand` as ZoneType].length}
                        </div>
        ...
        <>
            {isSignedIn && isClerkEnabled ? (
                ...
                <div className="bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold px-3 py-1 rounded-full transition-colors cursor-pointer border border-blue-400/30">
                    <SignInButton mode="modal" forceRedirectUrl="/practice" signUpForceRedirectUrl="/practice">ログイン</SignInButton>
                </div>
            ) : (
                <div className="bg-slate-800 text-slate-500 text-[10px] font-bold px-3 py-1 rounded-full border border-slate-700">
                    Offline
